...
- **Recuperación de contraseña (nuevo, commit `18b7ce0`):** no existía en ningún login. `/auth/recuperar` (pide email → Supabase envía enlace) y `/auth/nueva-contrasena` (define la clave y redirige a /admin o /portal por rol). Enlaces en ambos logins; el reset desde Admin → Agentes apuntaba a /portal/login (sin pantalla) y ahora a la nueva. `/auth` excluido del i18n en `proxy.ts`. En el proyecto Supabase nuevo faltaban Site URL y Redirect URLs (llevaban a localhost): configurados `https://assetsgolden.com` y `https://assetsgolden.com/**`. Probado en prod: solicitud para admin@ aceptada. Emails salen por el SMTP por defecto de Supabase (límite bajo/hora, puede caer en spam) — si se usa mucho, conectar Resend como SMTP en Auth.
- **CFG (gestion.cfg@gmail.com) compartió 3 carpetas de Drive** el 22/09 con las fotos de Gramado (50), Luque (25) y Córdoba (35): coincidían con 3 de las 14 pendientes. Descargadas con el conector de Drive (subagente, 110/110) y subidas con `uploadFolderPhotos.ts`. Destino Brasil completado con Gramado. Mapa en `scripts/output/cfg-drive-map.json`.
- **Regresión propia y arreglo:** el `--prune` de la auditoría podaba galerías según HEAD por URL, y Storage devuelve 500 transitorios bajo carga (backup de 4 GB): borró fotos reales de 64 galerías (Bali, Cervera, Fotocasa). Reconstruidas desde el listado real de Storage (`restore/<ref>/` y `cervera/<id>/`); Lovable no afectado. `--prune` retirado; la auditoría ahora compara contra el listado de Storage. Además `storage.list` con `limit 1000` devuelve menos objetos de los que hay y `offset>=1000` da `undefined`: paginar de 100. Y el pool del Micro se agota con listados rápidos ("Too many connections"): reintento con backoff + pausa por página (auditoría y backup). Tres corridas del backup completo fallaron por eso; relanzado con el fix.
- **Verificado contra Storage (fiable): 11 sin foto, 0 parciales, 158 completas.** Excel para Atilio regenerado: `../backups-migracion-2026-09/Fotos-pendientes-AssetsGolden-2026-09-22.xlsx`.
- **Destinos:** 4 de 5 arreglados con fotos vivas de propiedades del país (AR, DO, CR, EC). Brasil queda pendiente porque su única propiedad (Gramado) es una de las 14 sin foto.

**Próximo paso sugerido:** (1) Atilio: fotos de las 11 propiedades de `docs/fotos-pendientes.md` (+ Ana Serrat, 5 destinos) → subir con `scripts/uploadFolderPhotos.ts <ref> <carpeta> --confirm` o desde el panel. (2) Iván/Atilio/Joan: reset de contraseña en /admin/login y /portal/login; avisar a los 16 agentes. (3) Borrar el proyecto vacío sobrante de la org AssetGolden **verificando el ref** (el bueno es `ecvaqiotrzdjokaevsrz`). (4) Actualizar `docs/entrega-assets-golden.md` con el ref nuevo y el backup. (5) Descargar el primer artefacto de backup y guardarlo en Drive de AG.
...
- **Upstash (Fase 4b):** base Redis nueva en la cuenta de AG; Iván cambió `UPSTASH_REDIS_REST_URL/TOKEN` en Vercel y verificó el panel de admin tras el redeploy. **Resend:** Iván confirma que la cuenta ya es de Assets Golden → no se toca.
...
**Próximo paso sugerido:** Fase 5 (E2E) y Fase 6 (rotar clave de servidor de Supabase como Sensitive + JSON de la SA ya es nuevo; documento de entrega; sacar el usuario personal de Iván del team de Vercel). Después Fase 5 (E2E de los 6 formularios, portal PDF, crons) y Fase 6 (rotar service role + JSON de la SA, documento de entrega, sacar el usuario personal de Iván del team de Vercel para no pagar asiento extra). Pendiente menor: borrar el repo vacío `assetsgolden1/Webassetgolden`; re-loguear el CLI de Vercel local contra el team nuevo.
