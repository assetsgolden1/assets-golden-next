...
- **Tres fuentes automatizadas, aisladas entre sí:**
  ...
  - *Inmoges* (CRM del cliente, Expertos de Gestión): feed Kyero `https://expertosgestion.com/XMLky/3440.xml`. `npm run inmoges -- report | load --confirm [--fotos]` + workflow diario `sync-inmoges.yml` (05:00 UTC). Scope `external_source='inmoges'` + prefijo `im-`. **Sobre una ficha ya publicada solo toca el precio**: nunca pisa título, descripción ni fotos. Vínculos iniciales verificados a mano en `src/scripts/inmoges-match.json` (versionado: sin él el workflow duplicaría 12 fichas; la carga aborta si falta). Guía para el cliente: `docs/inmoges-crm-web.md`. **Activo desde el 24/09**: 35 fichas (23 creadas, 12 vinculadas).
